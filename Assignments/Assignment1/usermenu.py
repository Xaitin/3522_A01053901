...
class UserMenu:
    ...
    def __init__(self):
        self._user_accounts = UserList()

    def initialize_user_list(self):
        print("Please select the option that best applies to you: ")
        print("--------------------")
        print("1. User")
        my_input = input("2. Tester")
        if my_input == 1:
            self._user_accounts.create_account()
        elif my_input == 2:
            self._user_accounts.create_test_account()
    ...
    def display_user_menu(self):
        ...
        while user_input != 5:
            ...
            if user_input == 1:
                ...
                self._user_accounts.get_active_user().record_transaction(transaction_amount, transaction_type,
                                                                         transaction_location)
                self._user_accounts.get_active_user().show_transactions()
            elif user_input == 3:
                self._user_accounts.get_active_user().show_transactions()
            elif user_input == 4:
                # self._user_account.get_active_user().get_account_details()
                pass
            elif user_input == 5:
                pass
            else:
...
def main():
    ...
    my_epic_bank.initialize_user_list()
